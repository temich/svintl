Feature: Build
  ...
  Scenario: Build
    ...
    And the file `test/en-US.yaml` contains:
      ...
      hello: Hello
      """
    And the file `test/ru-RU.yaml` matches `^hello: (Здравствуйте|Привет)!?$`
    When I modify `test/ru-RU.yaml` to update `hello` to `Здравствуйте`
    And I modify `test/en-US.yaml` to update `hello` to `Hello2`
    ...
    And the file `test-foo/foo/en-US.yaml` contains:
      ...
      hello: Hello
      """
    And the file `test-foo/foo/ru-RU.yaml` matches `^hello: (Здравствуйте|Привет)!?$`
    When I modify `test-foo/foo/ru-RU.yaml` to update `hello` to `Здравствуйте`
    And I run `npx intl build -p ./test`
    Then the file `test-foo/foo/built.js` contains:
      """
      // Auto-generated by i18n CLI - do not edit manually
